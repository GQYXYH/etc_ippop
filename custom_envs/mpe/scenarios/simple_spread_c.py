...
class Scenario(BaseScenario):
    ...
    def action_callback(self, agent):
      ...
      if agent.action.c>0:
        ...
        # message_content =agent.state.p_pos
        # # should work guess
        # if len(self.message_queue) >= self.max_queue_length:
        ...
        # message_content.extend([0.0, 0.0])
    #   message_content = np.concatenate((message_content,[0]))
    #   if agent.action.c[0] <= agent.action.c[1]:
    #     message_content[-1]+=1
    
    #   self.message_queue.append((scheduled_time, agent.name, np.copy(message_content)))
    #   print(agent.action)
      return agent.action
    ...
    def observation(self, agent, world,current_step):
        ...
        for other in world.agents:
            if other is agent:
                ...
                other_pos.append(0.0)
            elif (other.action.c == 0) and other is not agent:
                other_pos.append(0.0)
                other_pos.append(0.0)
            elif (other.action.c == 1) and other is not agent:
                # print(self.bandwidth_limit)
                # self.communication_count += 1
                # print(other.name)
                if self.communication_counters[other.name] < self.bandwidth_limit:
                    
                    other_pos.append(other.state.p_pos - agent.state.p_pos)
                    # print(other_pos)
                    self.communication_counters[other.name] +=1
                else:
                    # Bandwidth limit exceeded, append zeros
                    other_pos.extend([0.0, 0.0])
        ...
        if np.random.rand() >= self.packet_drop_prob:#packet drops
            ...
            other_pos = np.zeros(2*self.n_agents+1)
        ...
        #     due_messages = self.process_message_buffer(current_step)
        #     for message, _, recipient in due_messages:
        #         if recipient == agent:
        #             # Process message to modify the agent's perception
        #             other_pos.append(message)
        # other_positions = np.array(other_pos)
        # print(current_step)
        # print(current_step,real_send_frame)
        # print("other_positions2",other_positions)
    ...
    def process_message_buffer(self, current_step, agent):
        ...
        return received_message if received_message is not None else np.zeros(2 * self.n_agents + 1)
