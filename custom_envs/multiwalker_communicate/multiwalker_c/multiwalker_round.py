...
class MultiWalkerEnv:
    ...
    def scroll_subroutine(self):
        ...
        for i in range(self.n_walkers):
            ...
            neighbor_obs = []
            if self.current_comm_index == 2:
            # This walker sends its data to all other walkers
                for j in range(self.n_walkers):
                    if (j == 0 or j == 1) and j!=i:
                        ...
                        neighbor_obs.extend([dxm, dym])
                    else:
                        neighbor_obs.extend([0.0, 0.0, 0.0, 0.0])  # Zero data for itself
            elif self.current_comm_index == 4:
            # This walker sends its data to all other walkers
                for j in range(self.n_walkers):
                    if (j == 2 or j == 3) and j!=i:
                        xm = (self.walkers[j].hull.position.x - x) / self.package_length
                        ym = (self.walkers[j].hull.position.y - y) / self.package_length
                        neighbor_obs.extend([xm, ym])
                        dxm = ((0.3*self.walkers[j].hull.linearVelocity.x*(VIEWPORT_W/SCALE)/FPS) - dx)
                        ...
                        neighbor_obs.extend([0.0, 0.0, 0.0, 0.0])  # Zero data for itself
            elif self.current_comm_index == 0:
            # This walker sends its data to all other walkers
                for j in range(self.n_walkers):
                    if (j == 4 or j == 5) and j!=i:
                        xm = (self.walkers[j].hull.position.x - x) / self.package_length
                        ym = (self.walkers[j].hull.position.y - y) / self.package_length
                        neighbor_obs.extend([xm, ym])
                        dxm = ((0.3*self.walkers[j].hull.linearVelocity.x*(VIEWPORT_W/SCALE)/FPS) - dx)
            ...
            # This walker is not communicating this step
                for j in range(self.n_walkers):
