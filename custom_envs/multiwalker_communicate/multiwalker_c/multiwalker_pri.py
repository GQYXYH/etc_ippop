...
class BipedalWalker(Agent):
    ...
    def action_space(self):
        ...
        return spaces.Box(
            low=np.array([-1, -1, -1, -1, 0]),   # 前四维是 -1，最后一维是 0
            high=np.array([1, 1, 1, 1, 1]),     # 前四维是 1，最后一维是 1
            dtype=np.float32
        )
...
class MultiWalkerEnv:
    ...
    def __init__(
        ...
        self.max_comm_signal = -float('inf')  # Initialize to a very low value
        self.max_comm_agent = None  # Store the index of the agent with the largest comm_signal
        self.prev_comm_signals = None
        self.current_comm_index = 0
        
    
    def send_message(self, message, target_agent_id, delay,loss_check):
        """Queue messages with specified delay if they are not dropped."""
        if loss_check >= self.packet_drop_prob:
            current_step = self.get_cycle_count()
            # print("here m",current_step)
    ...
    def send_priority(self, message, target_agent_id, delay,loss_check):
        """Queue messages with specified delay if they are not dropped."""
        if loss_check >= self.packet_drop_prob:
            current_step = self.get_cycle_count()
            # print("here p",current_step)
            deliver_at_step = current_step + delay
            # print(f"Appending message with current_step {current_step}, deliver_at_step {deliver_at_step}")
            self.priority_buffer.append((np.copy(message), deliver_at_step, target_agent_id))
            # print("len",len(self.message_buffer))
        if len(self.priority_buffer) > (self.num_agents*delay+self.num_agents):
    ...
    def process_priority_buffer(self, agent_id):
        """Process message buffer to deliver messages that are due and target the specified agent."""
        received_message = None  # Changed to None to handle no messages received case
        i = 0  # Initialize index counter
        current_step = self.get_cycle_count()
        ...
        while i < len(self.priority_buffer):
            message, deliver_at_step, target_agent_id = self.priority_buffer[i]
      
            if deliver_at_step == current_step and target_agent_id == agent_id:
               
                received_message = (message, deliver_at_step)
            #     # print("received_message",received_message)
                self.priority_buffer.pop(i)  # Remove the message from the buffer
            else:
                i += 1  # Only increment if no item was removed
            
        return received_message[0] if received_message is not None else 0
    ...
    def update_comm_signals(self):
        self.prev_comm_signals = [walker.comm_signal for walker in self.walkers]
    ...
    def scroll_subroutine(self):
        ...
        max_comm_signal = -float('inf')
        max_comm_agent = None
        loss_check = np.random.rand()
        if self.prev_comm_signals is None or all(signal == 0 or signal is None for signal in self.prev_comm_signals):
            max_comm_agent = self.current_comm_index % self.n_walkers
            self.prev_comm_signals = np.random.rand(self.n_walkers) 
        ...
        # print("pbuffer",self.priority_buffer)
        for i, signal in enumerate(self.prev_comm_signals):
            if signal > max_comm_signal:
                max_comm_signal = signal
                max_comm_agent = i
        
        # print(max_comm_agent,max_comm_signal)
        for i in range(self.n_walkers):
            # print("2")
            # if self.walkers[i].comm_signal>0.5:
            ...
            self.send_message(self.last_message[i],i,delay=self.delay,loss_check=loss_check)
        ...
        for i, signal in enumerate(self.prev_comm_signals):
            self.send_priority(signal, i, delay=self.delay,loss_check=loss_check)
        for i in range(self.n_walkers):
            self.prev_comm_signals[i] = self.process_priority_buffer(i)
        
        # print("send",self.priority_buffer)
    ...
    def step(self, action, agent_id, is_last):
        ...
        if is_last:
            ...
            # print("message",self.message_buffer)
    ...
    def render(self, close=False):
        if close:
            ...
            return
        front_walker_x = max(
            walker.hull.position.x for walker in self.walkers if walker.hull and walker.hull.position
        )
    
        # Adjust scroll to track the front walker
        self.scroll = front_walker_x - VIEWPORT_W / 2 / (SCALE / self.package_scale / 0.75)
        # min_x = min(walker.hull.position.x for walker in self.walkers if walker.hull and walker.hull.position)
        # max_x = max(walker.hull.position.x for walker in self.walkers if walker.hull and walker.hull.position)
        
        # # # Set self.scroll to the middle of the scene to center the view
        # self.scroll = (max_x + min_x) / 2

        # # Adjust render_scale to fit the entire width of the scene in the viewport
        # scene_width = max_x - min_x
        
        offset = 200  # compensates for the negative coordinates
        render_scale = SCALE / self.package_scale / 0.75
        # render_scale = VIEWPORT_W / (scene_width + 200)
