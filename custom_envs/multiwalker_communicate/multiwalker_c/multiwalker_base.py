...
class MultiWalkerEnv:
    ...
    def __init__(
        ...
        self.last_message = np.zeros((self.n_walkers,5+2*self.num_agents))
        self.message_buffer = []
    ...
    def process_message_buffer(self, agent_id, current_step):
        ...
        return received_message[0] if received_message is not None else np.zeros(5 + 2 * self.num_agents)
    ...
    def reset(self):
        ...
        for i in range(self.n_walkers):
            ...
            self.last_obs[i] = np.concatenate([o[i],np.zeros(5+2*self.num_agents)])
        self.frames = 0
        self.message_buffer = []
        self.last_message = np.zeros((self.n_walkers,5+2*self.num_agents))
        self.current_round_message_count = 0
    ...
    def scroll_subroutine(self):
        ...
        for i in range(self.n_walkers):
            # print("2")
            if self.walkers[i].comm_signal==1:
            ...
            for j in range(self.n_walkers):
                ...
                if j==i or self.walkers[j].comm_signal==0:
                    ...
                    if communication_count < self.bandwidth_limit:
                        # Communication happens, count this against the bandwidth limit
                        xm = (self.walkers[j].hull.position.x - x) / self.package_length
                        ym = (self.walkers[j].hull.position.y - y) / self.package_length
                ...
                #相对位置
                # print(neighbor_obs)
            assert len(neighbor_obs) == (2*self.n_walkers)
    ...
    def step(self, action, agent_id, is_last):
        ...
        if is_last:
            ...
            for i in range(self.n_walkers):
                ...
                assert self.last_obs[i].shape[0] == (self.num_agents*2+5+24)
            for i in range(self.n_walkers):
    ...
    def observe(self, agent):
        ...
        # print("o",o.shape)
        assert o.shape[0] == (24+2*self.num_agents+5)
        return o
