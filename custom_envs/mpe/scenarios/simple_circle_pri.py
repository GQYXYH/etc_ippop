...
from custom_envs.mpe.simple_env_pri import SimpleEnv, make_env
import itertools
...
class Scenario(BaseScenario):
    ...
    def reward(self, agent, world, global_reward=None):
        ...
        if agent.name == 'agent_0':
            ...
            rew += collision_rew
            # print(agent.action)
            rew -= np.clip(agent.action.c,0,1)*self.penalty_ratio
    ...
    def observation(self, agent, world,current_step,loss_check,send_message):
        ...
        #     max_comm_agent = current_comm_index % self.num_agents
        ...
        for i, signal in enumerate(send_message):
            if signal > max_comm_signal:
                max_comm_signal = signal
                max_comm_agent = world.agents[i]
        # print(max_comm_agent.name)
        # for i in range(self.n_agents):
        for other in world.agents:
            if other is agent:
                other_pos.append([0.0,0.0,0.0,0.0])
            elif other == max_comm_agent:
        ...
        other_pos_timer = list(itertools.chain.from_iterable(other_pos))
        # print("other_pos.shape[0]",other_pos_timer)
        other_pos = np.concatenate((list(itertools.chain.from_iterable(other_pos)),np.array([self.delay])))
        other_pos = np.concatenate((other_pos,[0]))
        # print(agent.action.c[0],agent.action.c[1])
        if all(value == 0 for value in other_pos_timer):
            other_pos[-1]+=1
        if loss_check >= self.packet_drop_prob:#packet drops
            pass
        else:
