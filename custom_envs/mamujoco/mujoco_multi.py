...
from custom_envs.mamujoco.obsk import (
    Node,
    build_obs,
    get_joints_at_kdist,
    get_parts_and_edges,
...
class MultiAgentMujocoEnv(pettingzoo.utils.env.ParallelEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # print("local_C", local_categories)
        if gym_env is None:
        ...
        # print("print(self.agent_obsk)",self.agent_obsk)
        ...
        # k_categories 的概念与 agent_obsk 参数直接相关，该参数指定代理在模拟中可以观察到多少个关节或部分连接级别（深度）。
        ...
        for agent_id, partition in enumerate(self.agent_action_partitions):
            ...
            #     dtype=np.float32,
            # )
            self.action_spaces[self.possible_agents[agent_id]] = gymnasium.spaces.Tuple([
                gymnasium.spaces.Box(low=self.single_agent_env.action_space.low[0],
                high=self.single_agent_env.action_space.high[0],
                shape=(len(partition),),
                dtype=np.float32),
    ...
    def _create_base_gym_env(
        ...
        if scenario in _MUJOCO_GYM_ENVIROMENTS:
            return gymnasium.make(f"{scenario}-v4", **kwargs, render_mode=render_mode)
    ...
    def step(
        ...
        # communication_action = actions[-1]
    ...
    # 三个环境 的mujoco_global是root. global_categories:qv,qp.
    # k_dicts到底有哪些附近的关节信息可以被捕捉到。
    # local_categories for humannoidstandup:[['qpos', 'qvel', 'cinert', 'cvel', 'qfrc_actuator', 'cfrc_ext'], ['qpos']]
    # Halfcheetah:[['qpos', 'qvel'], ['qpos']]. Ant:[['qpos', 'qvel', 'cfrc_ext'], ['qpos']]
