...
def build_obs(
    ...
    Args:
        ...
        k_dict：每个代理的字典，将观察深度 (k) 映射到可以从该深度观察到的代理（或其他代理）的部分。
        local_categories：指定在每个深度级别可见的数据类型。
        global_nodes：表示所有代理都可能观察到的数据点的节点，无论其本地位置如何,我们要用的环境是root。
        global_categories：与本地类别类似，但针对全局节点表示的数据点。
