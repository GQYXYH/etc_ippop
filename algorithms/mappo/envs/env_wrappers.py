...
def extract_non_empty_dicts(data):
    if isinstance(data, dict):
        # 对字典中的每一个值进行检查，如果有任何一个NumPy数组非空，即保留该字典
        if any(value.size > 0 if isinstance(value, np.ndarray) else value for value in data.values()):
            return data
    elif isinstance(data, tuple):
        # 对元组中的每一项递归处理
        return tuple(filter(None, (extract_non_empty_dicts(item) for item in data)))
class SubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        # 遍历字典中的项目
        # obs = tuple(filter(None, (extract_non_empty_dicts(item) for item in obs)))
