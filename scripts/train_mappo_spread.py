...
from custom_envs.mpe import simple_circle_c_v2
...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            ...
            if all_args.scenario_name == "simple_spread":
                env = simple_spread_c_v2.parallel_env(N=all_args.num_agents, penalty_ratio=all_args.com_ratio,max_cycles=25,
                    full_comm=all_args.full_comm, local_ratio=all_args.local_ratio, continuous_actions=True)
                env.reset(50000 + rank * 10000)
            elif all_args.scenario_name == "simple_formulation":
                # print("simple_formulation")
                env = simple_formulation_c_v2.parallel_env(N=all_args.num_agents, penalty_ratio=all_args.com_ratio,max_cycles=25,
                    full_comm=all_args.full_comm, local_ratio=all_args.local_ratio, continuous_actions=True)
                env.reset(50000 + rank * 10000)
            elif all_args.scenario_name == "simple_circle":
                # print("simple_formulation")
                env = simple_circle_c_v2.parallel_env(N=all_args.num_agents, penalty_ratio=0,max_cycles=25,
                    full_comm=all_args.full_comm, local_ratio=all_args.local_ratio, continuous_actions=True)
                env.reset(50000 + rank * 10000)
            return env
        return init_env
