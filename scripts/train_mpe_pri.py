...
def make_train_env(all_args):
    def get_env_fn(rank):
        def init_env():
            env = simple_circle_pri.parallel_env(N=8, penalty_ratio=all_args.com_ratio,max_cycles=25,
                    full_comm=False, local_ratio=all_args.local_ratio, continuous_actions=True,
                    delay = 1,packet_drop_prob=0.2,
                    bandwidth_limit = 10,landmarks = 1)
...
def main(args):
    ...
    # else:
    from algorithms.mappo.runner.separated.walker_pri import MPERunner as Runner
