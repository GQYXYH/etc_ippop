...
class R_MAPPO():
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        if self._use_policy_active_masks:
            ...
            # term_sqrt_ctrl_kl = ( torch.sum(sqrt_ctrl_kl,dim=-1,keepdim=True)* active_masks_batch).sum() / active_masks_batch.sum()
            # term_sqrt_com_kl = ( torch.sum(sqrt_com_kl,dim=-1,keepdim=True)* active_masks_batch).sum() / active_masks_batch.sum()

            # term_ctrl_kl  = (torch.sum(ctrl_kl,dim=-1,keepdim=True) * active_masks_batch).sum() / active_masks_batch.sum()
            # term_com_kl  = (torch.sum(com_kl,dim=-1,keepdim=True) * active_masks_batch).sum() / active_masks_batch.sum()
            ...
            if self.args.algorithm_name == 'ippo':
        ...
        if update_actor:
            if self.args.algorithm_name == 'ippo':
                # print("self.entropyppo_coef",self.entropyppo_coef)
