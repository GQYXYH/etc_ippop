...
class MultiWalkerEnv:
    ...
    def scroll_subroutine(self):
        ...
        # max_comm_signal = -float('inf')  # Initialize to a very low value
        # max_comm_agent = None  # Store the index of the agent with the largest comm_signal

        # for i in range(self.n_walkers):
        #     # if self.walkers[i].hull is not None:  # Ensure the agent is valid
        #     if self.walkers[i].comm_signal > max_comm_signal:
        #         max_comm_signal = self.walkers[i].comm_signal
        #         max_comm_agent = i
        # print(max_comm_signal,max_comm_agent)
        max_comm_signal = -float('inf')
        second_max_comm_signal = -float('inf')

        # Store the indices of the agents with the largest and second largest comm_signal
        max_comm_agent = None
        second_max_comm_agent = None

        for i in range(self.n_walkers):
            # Ensure the agent is valid (uncomment the condition if needed)
            # if self.walkers[i].hull is not None:
            if self.walkers[i].comm_signal > max_comm_signal:
                # Update second max to current max
                second_max_comm_signal = max_comm_signal
                second_max_comm_agent = max_comm_agent

                # Update max to new value
                max_comm_signal = self.walkers[i].comm_signal
                max_comm_agent = i
            elif self.walkers[i].comm_signal > second_max_comm_signal:
                # Only update second max if it's greater than the current second max but less than the max
                second_max_comm_signal = self.walkers[i].comm_signal
                second_max_comm_agent = i
        # print(max_comm_agent,second_max_comm_agent)
        for i in range(self.n_walkers):
            # print("2")
            ...
            for j in range(self.n_walkers):
                ...
                elif j == max_comm_agent or j == second_max_comm_agent:
